...
pattern_homol <- c("concedo a recuperacao judicial", "o plano deve ser homologado","o plano de recuperacao judicial deve ser homologado", "homologo o plano") %>% paste(collapse = "|")
...
hom <- hom_filtro %>% 
  bind_rows(hom_homol, hom_art58, hom_art57) %>% 
  group_by(processo) %>% 
  slice(1) %>% 
  ungroup()
...
```{r exemplos de dados que chamaram a atenção}



ex1 <- movs_df %>% filter(id_linha == 31528) %>% pull(detalhes)
#processamento deferido em 24/03/2009
#teve assembleia geral de credores depois
#plano foi aprovado em AGC
#"decido, o plano deve ser homologado". Decisão dia 2010-10-06



#caso que aparece quando filtramos pelo artigo 58 (que fala da homologação do PRJ), mas a movimentação é uma decretação de falência
#id_linha dele é 2996


ex2 <- movs_df %>% filter(id_linha == 2996)
# ex2 %>% pull(processo)
...
pattern_73 <- c("73, iii, | 73, inc. iii, | 73, inciso iii, | 73, ii, | 73, inc. ii, | 73, inciso ii,")
...
#decretação de falência não aparece nos movimentos digitalizados

#1004044-30.2017.8.26.0073
#linha 7069: cita art. 99.
#vou ver pq art. 99 não pegou esse processo


#0008645-88.2012.8.26.0100
#4042: decretacao, por sentenca, do encerramento da recuperacao judicial
