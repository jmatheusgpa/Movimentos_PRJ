...
#Filtrarmos a primeira observação que a cita a palavra chave em cada processo,
#pois às vezes um documento posterior citava trechos da decisão. 
#Por exemplo, publicavam um edital dizendo que a decisão citou "xyz" no dia tal e tal.
#isso é comum em documentos do tipo Edital ou Certidão de Objeto e Pé, que registram o andamento dos processos
#parece então que podemos resolver o problema pegando apenas a primeira movimentação de cada grupo
...
hom <- hom_filtro %>% 
  bind_rows(hom_homol, hom_art_99_73, hom_homol_regex, hom_nahom) %>% 
  group_by(processo) %>% 
  slice(1) %>% 
  ungroup()
...
hom_homol2 <- movs_df %>% 
  procura_resume(" bem conceder a recuperacao judicial") %>% 
  arrange(processo, date) %>% 
  group_by(processo) %>% 
  slice(1) %>% 
  ungroup()
...
hom_homol4 <- movs_df %>% 
  procura_resume('entendo por bem conceder a recuperacao judicial') %>% 
  arrange(processo, date) %>% 
  group_by(processo) %>% 
  slice(1) %>% 
  ungroup()
...
pattern_nafinal_enc <- c('vem a conclusao com requerimento da administradora judicial para encerramento, por forca do cumprimento das obrigacoes vencidas') %>% paste(collapse = "|")
...
#checando cada processo que falta data de homologação ####
...
#0000547-55.2014.8.26.0291 #tem homologação
#225
...
#0047431-70.2013.8.26.0100 #tem homologação 
#170806
...
# 1000073-74.2014.8.26.0224
# plano foi rejeitado por uma classe e juiz preferiu convolar em falência

# 1000186-82.2016.8.26.0539
# plano rejeitado

# 1000414-47.2016.8.26.0219
# não chegou a votar o plano

# 1000464-86.2016.8.26.0538
# não chegou a votar o plano (ou isso não aparece na movimentação)


#1000718-73.2017.8.26.0037
# não chegou a votar o plano

#1000989-12.2015.8.26.0180
# não chegou a votar o plano


#1001774-95.2014.8.26.0248
#não chegou a votar o plano

#1002139-25.2015.8.26.0278
#plano rejeitado

#1002196-77.2014.8.26.0666
#plano rejeitado

#1002315-50.2014.8.26.0565
#não chegou a votar o plano

#1002317-10.2013.8.26.0127
#não chegou a votar o plano

#1002593-72.2016.8.26.0115
#não chegou a votar o plano

#1002736-60.2016.8.26.0568
#não chegou a votar o plano

#1002781-81.2014.8.26.0100
#credores apresentaram objeção e empresa decidiu falir antes de tentar votar

#1004159-59.2016.8.26.0114 #tem homologação
#71114
#ciencia a todos os interessados sobre o plano aprovado


#1004418-81.2015.8.26.0278 #tem homologação
#não tem movimento sobre o plano ter sido aprovado


#1004718-69.2016.8.26.0161
#não chegou a votar o plano

#1005217-59.2014.8.26.0408
#75472
...
#1005245-54.2016.8.26.0344
#não chegou a votar o plano

#1005397-07.2017.8.26.0526
#não encontrei homologação do plano

#1006752-59.2013.8.26.0278
#não chegou a votar o plano

#1006879-94.2013.8.26.0278
#não chegou a votar o plano

#1011894-65.2016.8.26.0625
#não chegou a votar o plano

#1012224-51.2014.8.26.0037
#não encontrei data de homologação nos movimentos digitalizados
#chequei pdf e.. não chegou a votar o plano

#1014271-94.2017.8.26.0068
#não chegou a votar o plano
...
#1048371-81.2014.8.26.0100
#não chegou a votar o plano

#1056096-53.2016.8.26.0100
#não chegou a votar o plano

#1059314-13.2017.8.26.0114
...
# resumo ####
#quais processos resultaram em falência e que ainda não tabulei a data de homologação?

#1022188-11.2015.8.26.0562 
#não consegui checar se tem ou não homologação do plano
#o texto completo da decisão de decretação de falência mostra que o plano não chegou a ser aprovado. Usarei a data de falência como data de rejeição do plano


#1017667-88.2016.8.26.0529 
#teve plano de recuperação, conforme a sentença da página 840 do pdf. Não achei a aprovação do plano por enquanto
#Na verdade não teve plano de recuperação aprovado. O relatório do AJ na página 790 do pdf diz que não haviam sequer publicado edital para convicação de AGC, e após isso o juiz decreta a falência.
...
#1004418-81.2015.8.26.0278
#não tem movimento sobre o plano ter sido aprovado
#relatório da decisão do juiz dia 29/01/2019 cita que a empresa não chegou a ter plano aprovado. Podemos usar falência como data de rejeição do plano


#0047431-70.2013.8.26.0100
#170806
#o plano foi aprovado em assembleia geral de credores e a recuperacao foi concedida
#não encontrei data de homologação, só tem data de acordo com o BB
#olhei o processo duas vezes e não achei decisão que homologasse o plano. Podemos usar falência como data de rejeição
#na página 655 do pdf: adm judicial fala que PRJ foi rejeitado e aguarda convolação em falência
...
checagem('1013766-34.2016.8.26.0361')


```


```{r casos a serem descartados}
...
descartar <- c('1097074-09.2015.8.26.0100', '1000374-40.2015.8.26.0562', '1004272-50.2016.8.26.0037', '1013766-34.2016.8.26.0361','1108161-88.2017.8.26.0100', '1012203-08.2016.8.26.0554', '1012203-08.2016.8.26.0554', '1023746-04.2015.8.26.0114', '1025824-13.2015.8.26.0100','1008112-48.2015.8.26.0152', '0024997-87.2013.8.26.0100')
...
```{r casos com encerramento após 2018}

#aqui teve uma mistura dos casos que eu filtrei na reorganizations_court_info_csv


datas_rj %>% 
  filter(data_final > '2019-12-31')



#casos do input manual que foram encerrados

# input_manual %>% 
#   filter(outcome %in% c('Recuperacao Judicial foi convolada em falencia','Recuperacao Judicial foi encerrada')) %>%
#   tally() #tem 197 casos. Então da onde vieram os 274 casos?
