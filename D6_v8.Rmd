...
pattern_nahom <- c('ciencia a todos os interessados sobre o plano aprovado') %>% paste(collapse = "|")
...
hom_homol4 <- movs_df %>% 
  procura_resume('ciencia a todos os interessados sobre o plano aprovado') %>% 
  arrange(processo, date) %>% 
  group_by(processo) %>% 
  slice(1) %>% 
  ungroup()
...
#checando se faz diferença no resultado final

hom2 <- hom_filtro %>% 
  bind_rows(hom_homol, hom_art_99_73, hom_homol_regex, hom_homol4) %>% 
  group_by(processo) %>% 
  slice(1) %>% 
  ungroup()
...
#checa quais movimentações do processo x possuem as palavras-chave inseridas em 'procura_resume'  
  check <- movs_df %>%
  filter(processo == x) %>% 
  procura_resume("plano | aprov | homolog | assembleia")
...
checagem('1004159-59.2016.8.26.0114')
