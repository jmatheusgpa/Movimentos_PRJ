...
#precisamos deixar apenas as datas que ocorreram antes da homologação/rejeição do plano, pois esta marca o final do jogo
data_hom <- datas_rj %>% 
  select(processo, data_hom)
...
datas_rj %>% 
  ...
  summarise(mean_req_hom = median(meses_req_hom))
...
reorg_sem_obj <- datas_rj %>%
  filter((tem_obj == 0) & (resultado == 'encerrou RJ com êxito'))
```
...
```{r lista de processos com dados de movimentação}

lista_mov <- datas_rj %>% 
  select(processo)

# write.xlsx(lista_mov, file = 'lista_mov.xlsx')
...
datas_rj_export <- datas_rj %>% 
  select(processo, data_requerimento, data_hom, data_final, resultado, meses_req_hom, meses_req_final, meses_hom_final, tem_obj, avg_months_decisions) %>% 
  rename(filing_date = data_requerimento, 
         plan_date = data_hom, #data de confirmação ou rejeição do plano
         final_date = data_final, 
         outcome = resultado, 
         months_fil_plan = meses_req_hom, 
         months_req_final = meses_req_final,
...
# teste <-  read.csv('datas_rj.csv', fileEncoding = 'utf8')
