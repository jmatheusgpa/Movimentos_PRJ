...
##### Data de requerimento/ Filing date
A data inicial de cada processo de recuperação inicial será a data de quando a empresa fez o requerimento da recuperação judicial. A nossa proxy para isso é a data da primeira movimentação oficial no caso, usualmente a data de distribuição do processo.
...
#### Data de deferimento da RJ/Start date

Data quando o juiz aprova a documentação apresentada pela empresa e concede o stay period. Regulada pelo artigo 152 da Lei 11.101/2005.
Esse é o começo do período analisado pelo modelo.
...
pattern_defiro <- c("defiro o processamento",
                    "processamento deferido") %>% paste(collapse = " | ")


def_pattern <- movs_df_completo %>% 
...
pattern_art52 <- c("artigo 52",
                    "art. 52", 'art.52') %>% paste(collapse = "|")

def_art52 <- movs_df_completo %>% 
...
#existe um processo que não tem movimentação com os padrões acima
#usaremos o artigo 7º para este (artigo que fala do edital da relação de credores)
pattern_art7 <- c('artigo 7º')

def_art7 <- movs_df_completo %>% 
  procura_resume(pattern_art7) %>% 
  arrange(processo, date) %>% 
  group_by(processo) %>% 
  slice(1) %>% 
  ungroup()
...
#juntando as bases e selecionando as colunas desejadas
def <-  def_art52 %>% 
  bind_rows(def_pattern, def_art7) %>% 
  arrange(processo, date) %>% 
  group_by(processo) %>% 
  slice(1) %>% 
  ungroup() %>% 
...
def <- def %>% 
  rename(data_start = date,
         detalhes_start = detalhes,
         detalhes_start_resumo = detalhes_resumo,
         id_linha_start = id_linha)
...
```{r datas_rj - adicionando datas de deferimento(start) e finalização}
...
datas_rj <- primeira_mov %>% 
  ...
  rename(data_req = date)

#incorporando data final do processo ####
datas_rj <- datas_rj %>% 
  left_join(final, by = "processo")

#incorporando data de deferimento do processo

datas_rj <- datas_rj %>% 
  left_join(def, by = 'processo')


# #checando se falta alguma data de deferimento
# datas_rj %>% 
...
Essa data marca o final do período analisado pelo modelo usado na minha dissertação.
...
#testando caso 202-38, que não tem objeção, mas classifiquei erroneamente que tinha

#textos específicos que encontrei olhando casos que não tinham objeção
pattern_naoobj <- c("nao houve qualquer objecao", 'inexistencia de objecao', 'nao havendo objecao', 'nao houve objecao') %>% paste(collapse = '|')



datas_rj <- datas_rj %>% 
  mutate(naoobj = str_detect(detalhes_hom, pattern_naoobj))

check <- datas_rj %>% 
  filter(naoobj==T)

datas_rj <- datas_rj %>% 
  mutate(tem_obj = case_when(naoobj==T~0,
                             TRUE~tem_obj))
...
#### Checando se há planos aprovados por cram down

```{r buscando cram down}
#só vai ter problema se o plano foi aprovado por cram down e a empresa for reorganizada, então vamos filtrar por estes critérios

cram_reorg <- datas_rj %>% 
  filter(resultado == "reorganized") %>% 
  filter(str_detect(detalhes_hom, 'cram'))

#os dois casos citam "cram" nos detalhes da homologação, porém não são casos em que o juiz usou cram down. Logo, está ok para a estimação
...
#### Número de meses entre agcs

Proxy para 'number of months between observed proposals'. Ainda será necessário filtrar para os casos que acabaram dentro da corte


```{r número de meses entre agcs}
#outra proxy para o número de meses entre planos observados
#temos que calcular o número médio de meses entre agcs e o número de agcs por processo
#isso permitirá calcular a média incondicional posteriormente
...
#calculando  número de meses entre um movimento citando agc e outro

#filtro para movimentos que podem ser duplicados
agc_duplicado <- c('Remetido ao DJE', 'Autos no Prazo','Data da Publicação SIDAP')

months_agc <- agc_semslice %>% 
  filter(!(movimento %in% agc_duplicado)) %>%
  select(processo, date, detalhes, id_linha) %>%
...
#calculando meses
#filtramos número de meses acima de 1 para novamente evitar movimentos duplicados
#isto é, um movimento que apenas cita 'agc', embora não se trate de uma agc ou uma decisão do juiz
months_agc <- months_agc %>%
  group_by(processo) %>% 
  mutate(n_months = (date - date_lag)/30) %>% 
  mutate(n_months = as.numeric(n_months)) %>% 
  filter(n_months > 1) %>% 
  ungroup()
  

#removendo datas que estão fora do período analisado pelo jogo
#isto é, datas que tenham ocorrido antes de a RJ ser deferida ou depois da data de homologação
data_hom <- datas_rj %>% 
  select(processo, data_hom, data_start)


months_agc <- months_agc %>%
  left_join(data_hom) %>% 
  filter(date_lag >= data_start & date < data_hom)



#passando log nas variaveis
months_agc <- months_agc %>%
  mutate(ln_n_months = log(n_months)) %>% 
  select(processo, n_months, ln_n_months)


#escrevendo arquivo para cruzar com outras bases




#testando
#calculando a média do log de meses entre movimentos com agc para todos os processos
teste <- months_agc %>% 
  pull(ln_n_months) %>% mean()
...
### Quantidade de AGCs no processo e as decisões de deferir e homologar

```{r agc + deferimento + decisão sobre o PRJ}
...
timing_agc <- numerode_agc %>%
  left_join(data_hom) %>% 
  filter(!(is.na(data_hom))) %>% 
  mutate(tempo_total = data_hom - data_start) %>% 
  mutate(tempo_medio_agc = tempo_total / ((n) * 30)) %>% 
  mutate(tempo_medio_agc = as.numeric(tempo_medio_agc))


t1 <- summary(timing_agc$tempo_medio_agc)
  


#passando log nas variaveis
timing_agc <- timing_agc %>%
  mutate(ln_n_months = log(tempo_medio_agc)) %>% 
  select(processo, tempo_medio_agc, ln_n_months)
...
#testando se há agcs após a data de homologação ####

agc_apos_homol <- months_agc %>% 
  left_join(data_hom) %>% 
  filter(date > data_hom)


#tem sim

#como ficam as estatísticas se removermos as agcs após a homologação?
months_agc <- months_agc %>%
  group_by(processo) %>% 
  mutate(n_months = (date - date_lag)/30) %>% 
  mutate(n_months = as.numeric(n_months)) %>% 
...
t2 <- summary(timing_agc$tempo_medio_agc)
...
#diferença entre requerimento e deferimento (start):tempo antes de o juiz conceder stay period
datas_rj <- datas_rj %>% 
  mutate(dias_req_start = (data_start - data_req),
         meses_req_start = dias_req_start/30)


#diferença entre deferimento (start) e homologação:tempo analisado pelo jogo
datas_rj <- datas_rj %>% 
  mutate(dias_start_hom = (data_hom - data_start),
         meses_start_hom = dias_start_hom/30)

#diferença entre homologação e finalização da RJ: tempo após confirmação/rejeição do plano e finalização do processo
datas_rj <- datas_rj %>% 
  mutate(dias_hom_final = (data_final - data_hom),
         meses_hom_final = dias_hom_final/30)


#diferença entre requerimento e finalização da RJ: tempo total que passa na corte
datas_rj <- datas_rj %>% 
  mutate(dias_req_final = (data_final - data_req),
...
datas_rj_descr <- datas_rj %>% select(meses_req_final, meses_req_start, meses_start_hom, meses_hom_final) %>% as.data.frame()
...
colunas_descr <- c("Meses entre requerimento e final", "Meses entre requerimento e deferimento", 'Meses entre deferimento e homologação', "Meses entre homologação e final")
...
datas_rj %>% 
  group_by(resultado) %>% 
  summarise(mean_req_hom = median(meses_start_hom))
...
Após o deferimento da recuperação judicial, o juiz suspende todas as cobranças contra a empresa pelo prazo de 180 dias. Neste período espera-se que a empresa apresente um plano de recuperação judicial e consiga apoio de seus credores. Então destacamos o prazo de 6 meses na linha tracejada no gráfico. Percebe-se que a minoria dos casos cai dentro desse prazo "ideal".



```{r g2}

#gráfico de meses entre start e homologação
#usei escala de 12 em 12 para facilitar a visualização dos dados em anos.
#o argumento binwidth = 12 dentro de geom_histogram() não funcionou como eu esperava

xmin2 <- datas_rj %>% pull(meses_start_hom) %>% as.numeric() %>% min(na.rm = T) %>% floor()
xmax2 <- datas_rj %>% pull(meses_start_hom) %>% as.numeric() %>% max(na.rm = T) %>% floor()

  
datas_rj %>% 
  ggplot()+
  geom_density(aes(x = meses_start_hom), na.rm = T, fill = "blue", alpha = 0.3, color = "black") +
  scale_x_continuous(breaks = seq(0, xmax2, 12))+
  xlab("Meses") +
  geom_vline(aes(xintercept = 6), linetype="dashed", colour="red") +
  ggtitle("Número de meses entre deferimento e homologação do processo")+
  ylab("Densidade")+
  theme_economist_white(base_size = 8, base_family = "sans", horizontal = F) +
  theme(plot.title = element_text(hjust = 0.5, size = 11)) +
  xlab("Meses")
...
Agora o mínimo que temos entre requerimento e homologação do processo é de `r datas_rj$meses_start_hom %>% as.numeric() %>% min()` meses.  Isso faz sentido, pois a firma tem 60 dias para apresentar o plano de recuperação e os credores têm mais 30 dias para apresentar qualquer objeção. Então teoricamente o número mínimo de meses que poderíamos observar é 3.

Prazos muitos altos entre deferimento e homologação ou foram porque o processo demorou mesmo ou porque a empresa faliu após ter o plano rejeitado pelos credores. Nos dois casos, tem uma demora.
...
#gráfico de meses entre homologação e final
#usei escala de 12 em 12 para facilitar a visualização dos dados em anos.
...
reorg_obj <- datas_rj %>%
  filter((tem_obj == 1) & (resultado == 'reorganized'))
...
reorg_obj_export <- reorg_obj %>% 
  select(processo, data_agc, data_hom, detalhes_hom)


# write.xlsx(reorg_obj_export, file = "reorg_obj.xlsx")


#lista sem objecao que foi reorganizado: 9 observações
reorg_sem_obj <- datas_rj %>%
  filter((tem_obj == 0) & (resultado == 'reorganized'))
```
...
```{r lista de processos com objeção}

lista_obj <- datas_rj %>% 
  select(processo, tem_obj) %>% 
  filter(tem_obj == 1)

# write.xlsx(lista_obj, file = 'lista_obj.xlsx')
...
write.xlsx(timing_agc, file = 'months_agc.xlsx')
...
```{r banco de dados com as datas das movimentações}

#resumindo o banco de dados, mantendo só os dados que serão usados

datas_rj_export <- datas_rj %>% 
  select(processo, data_req, data_start, data_hom, data_final, resultado, meses_req_start, meses_start_hom, meses_hom_final, meses_req_final, tem_obj) %>% 
  rename(fil_date = data_req, 
         plan_date = data_hom, #data de confirmação ou rejeição do plano
         final_date = data_final, 
         outcome = resultado, 
         months_fil_start = meses_req_start,
         months_start_plan = meses_start_hom,
         months_plan_final = meses_hom_final,
         months_fil_final = meses_req_final,
         in_court = tem_obj)
         

write.csv(x = datas_rj_export, file = "datas_rj_export.csv", fileEncoding = 'utf8')

# teste <-  read.csv('datas_rj_export.csv', fileEncoding = 'utf8')
