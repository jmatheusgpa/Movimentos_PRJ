...
#### Número de meses entre as decisões do juiz

```{r meses entre decisões do juiz}

#proxy para simular o quanto demora um período dentro do jogo

#usarei todas as movimentações que possivelmente são feitas pelo juiz, coletadas manualmente olhando 'descr_movimentos'

mov_juiz <- c("Decisão",'Decisão Determinação', "Mero expediente",'Despacho',"Despacho Proferido", 'Ato ordinatório',"Sentença Completa com Rsolução de Mérito",'Sentença Declarada', "Sentença de Concessão Completa", "Sentença de Concessão Resumida", 'Sentença  Proferida', "Declarado o Encerramento da Falência", "Decretação de falência", "Decretação do Encerramento da Recuperação Judicial", "Decretada a Falência - Sentença Completa", "Decretada a Falência - Sentença Resumida", "Concedida a Antecipação de tutela", "Concedida a Dilação de Prazo", "Concedida a Medida Liminar", "Concedida a recuperação judicial", "Concedida em parte a Antecipação de Tutela","Concedido efeito suspensivo a Recurso")

months_decisions <- movs_df_completo %>% 
  filter(movimento %in% mov_juiz) %>% 
...
months_decisions <- months_decisions %>%
  group_by(processo) %>% 
  mutate(n_months = (date - date_lag)/30)


#precisamos deixar apenas as datas que ocorreram após o deferimento(start) antes da homologação/rejeição do plano, 
#pois estas marcam o início e o final do jogo
data_hom <- datas_rj %>% 
  select(processo, data_hom, data_start)

months_decisions <- months_decisions %>%
  left_join(data_hom) %>% 
  filter(date >= data_start & date <= data_hom)

#limpando a base

months_decisions <- months_decisions %>% 
  group_by(processo) %>% 
  summarize(avg_months_decisions = mean(n_months)) %>% 
  ungroup()



#juntando os dados na base
datas_rj <- datas_rj %>% 
  left_join(months_decisions)
```
...
```{r lista de processos reorganizados sem objeção}

lista_reorg_precourt <- reorg_sem_obj %>% 
  select(processo)

# write.xlsx(lista_reorg_precourt, file = 'lista_reorg_precourt.xlsx')
...
datas_rj_export <- datas_rj %>% 
  select(processo, data_req, data_start, data_hom, data_final, resultado, meses_req_start, meses_start_hom, meses_hom_final, meses_req_final, tem_obj, avg_months_decisions) %>% 
